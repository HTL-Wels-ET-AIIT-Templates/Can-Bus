...
//            externer CAN-Transei ver wird ben�tigt
//
...
// Sender und Empf�nger m�ssen die gleiche Datenrate haben
//--------------------------------------------------------------
...
// PORT , PIN      , Clock              , Source
  {GPIOB,GPIO_Pin_9,RCC_AHB1Periph_GPIOB,GPIO_PinSource9}, // TX an PB9 -> Probleme beim Senden, LCD rauscht
  {GPIOB,GPIO_Pin_8,RCC_AHB1Periph_GPIOB,GPIO_PinSource8}, // RX an PB8 -> Probleme beim Senden, LCD rauscht
//  {GPIOB,GPIO_Pin_1,RCC_AHB1Periph_GPIOD,GPIO_PinSource1}, // TX an PD1 -> nicht verwendbar, kein sauberes Signal
//  {GPIOB,GPIO_Pin_0,RCC_AHB1Periph_GPIOD,GPIO_PinSource0}, // RX an PD0 -> nicht verwendbar, kein sauberes Signal
//  {GPIOA,GPIO_Pin_12,RCC_AHB1Periph_GPIOA,GPIO_PinSource12}, // TX an PA12 -> passt (lt. Beschreibung auch LCD_R4) 
//  {GPIOA,GPIO_Pin_11,RCC_AHB1Periph_GPIOA,GPIO_PinSource11}, // RX an PA11 -> passt (lt. Beschreibung auch LCD_R5)
};
...
ErrorStatus UB_CAN1_send_std_data(CAN1_TX_FRAME_t tx_frame)
{
  ...
  CAN1_TxMessage.DLC = tx_frame.nr_bytes;
...
CAN1_STATUS_t UB_CAN1_receive(CAN1_RX_FRAME_t *rx_frame)
{
  ...
  if(can1_buffer.status==CAN1_RX_READY) {
    ...
    rx_frame->nr_bytes=can1_buffer.nr_bytes;
    for(n=0;n<8;n++) {
...
// falls ein Filter nicht ben�tigt wird,
...
void UB_CAN1_std_FilterList(CAN1_STD_FL_t filter, uint8_t nr)
{
  ...
  // CAN Filter
  CAN1_FilterInitStructure.CAN_FilterNumber = nr;
  CAN1_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdList;
  CAN1_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_16bit;
  CAN1_FilterInitStructure.CAN_FilterIdHigh = P1_make_stdFL(filter,2);     // ID2
  CAN1_FilterInitStructure.CAN_FilterIdLow = P1_make_stdFL(filter,1);      // ID1
  CAN1_FilterInitStructure.CAN_FilterMaskIdHigh = P1_make_stdFL(filter,4); // ID4
  CAN1_FilterInitStructure.CAN_FilterMaskIdLow = P1_make_stdFL(filter,3);  // ID3
...
//
// Ein CAN-Frame kommt nur durch, wenn seine ID+Mode
// mit Hilfe der Maske �bereinstimmt
//
// nr = [0...13] (f�r CAN1)
//--------------------------------------------------------------
void UB_CAN1_std_FilterMask(CAN1_STD_FM_t filter, uint8_t nr)
{
  ...
  CAN1_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_16bit;
  CAN1_FilterInitStructure.CAN_FilterIdHigh = P1_make_stdFM(filter,3);     // ID2
  CAN1_FilterInitStructure.CAN_FilterIdLow = P1_make_stdFM(filter,1);      // ID1
  CAN1_FilterInitStructure.CAN_FilterMaskIdHigh = P1_make_stdFM(filter,4); // Maske2
  CAN1_FilterInitStructure.CAN_FilterMaskIdLow = P1_make_stdFM(filter,2);  // Maske1
  CAN1_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
  CAN1_FilterInitStructure.CAN_FilterActivation = ENABLE;
  CAN_FilterInit(&CAN1_FilterInitStructure);
}
...
void UB_CAN1_ext_FilterList(CAN1_EXT_FL_t filter, uint8_t nr)
{
  ...
  CAN1_FilterInitStructure.CAN_FilterIdHigh = P1_make_extFL(filter,2);     // ID1_Hi
  CAN1_FilterInitStructure.CAN_FilterIdLow = P1_make_extFL(filter,1);      // ID1_Lo
  CAN1_FilterInitStructure.CAN_FilterMaskIdHigh = P1_make_extFL(filter,4); // ID2_Hi
  CAN1_FilterInitStructure.CAN_FilterMaskIdLow = P1_make_extFL(filter,3);  // ID2_Lo
  CAN1_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
  CAN1_FilterInitStructure.CAN_FilterActivation = ENABLE;
  CAN_FilterInit(&CAN1_FilterInitStructure);
}
...
void UB_CAN1_ext_FilterMask(CAN1_EXT_FM_t filter, uint8_t nr)
{
  ...
  CAN1_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;
  CAN1_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;
  CAN1_FilterInitStructure.CAN_FilterIdHigh = P1_make_extFM(filter,2);     // ID1_Hi
  CAN1_FilterInitStructure.CAN_FilterIdLow = P1_make_extFM(filter,1);      // ID1_Lo
  CAN1_FilterInitStructure.CAN_FilterMaskIdHigh = P1_make_extFM(filter,4); // Mask1_Hi
  CAN1_FilterInitStructure.CAN_FilterMaskIdLow = P1_make_extFM(filter,3);  // Mask1_Lo
  CAN1_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
  CAN1_FilterInitStructure.CAN_FilterActivation = ENABLE;
  CAN_FilterInit(&CAN1_FilterInitStructure);
}
...
void P1_init_CAN(void) {
  ...
  // und zus�tzlich die in CAN_FilterIdHigh vorgegebene ID
  // Filtergleichung:
  // (ID & CAN_FilterMaskIdHigh/Low) == CAN_FilterIdListHigh/Low
  // -> CAN-Pakete d�rfen passieren
  //
  // Bedeutung der Maske:
  // 0x000 ... alle Bit durchlassen 
  // 0x7ff ... alle Bit sperren 
  // 0x6ff ... alle Bit durchlassen, f�r die das 9.Bit gesetzt ist  

  CAN1_FilterInitStructure.CAN_FilterNumber = 0; // erste Nr fuer CAN1
  CAN1_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;
  CAN1_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;
...
void P1_receive(void)
{
  ...
  if(can1_buffer.status==CAN1_RX_EMPTY) {
    can1_buffer.nr_bytes=CAN1_RxMessage.DLC;
