...
//  {GPIOB,GPIO_Pin_6,RCC_AHB1Periph_GPIOB,GPIO_PinSource6}, // TX an PB6 -> LCD geht nicht
//  {GPIOB,GPIO_Pin_5,RCC_AHB1Periph_GPIOB,GPIO_PinSource5}, // RX an PB5 -> LCD geht nicht
  {GPIOB,GPIO_Pin_13,RCC_AHB1Periph_GPIOB,GPIO_PinSource13}, // TX an PB13
  {GPIOB,GPIO_Pin_12,RCC_AHB1Periph_GPIOB,GPIO_PinSource12}, // RX an PB12
};
...
// nr_bytes = Anzahl der Daten die gesendet werden
...
ErrorStatus UB_CAN2_send_std_data(CAN2_TX_FRAME_t tx_frame)
{
  ...
  CAN2_TxMessage.DLC = tx_frame.nr_bytes;

  for(n=0;n<tx_frame.nr_bytes;n++) {
    CAN2_TxMessage.Data[n] = tx_frame.data[n];
  }

  // warten bis ein Transmit Register frei ist
  timeout=0;
  while(!(CAN2->TSR & CAN_TSR_TME0 || CAN2->TSR & CAN_TSR_TME1 || CAN2->TSR & CAN_TSR_TME2)) {
    timeout++;
    if(timeout>CAN2_TX_TIMEOUT)  break;
  }
...
// tx_frame :
// can_id    = Message Identifier [0...0x7FF]
// nr_bytes = Anzahl der Daten die erwartet werden
...
CAN2_STATUS_t UB_CAN2_receive(CAN2_RX_FRAME_t *rx_frame)
{
  ...
  if(can2_buffer.status==CAN2_RX_READY) {
    ...
    rx_frame->nr_bytes=can2_buffer.nr_bytes;
    for(n=0;n<8;n++) {
...
void UB_CAN2_std_FilterList(CAN2_STD_FL_t filter, uint8_t nr)
{
  ...
  // CAN Filter
  CAN2_FilterInitStructure.CAN_FilterNumber = nr;
  CAN2_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdList;
  CAN2_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_16bit;
  CAN2_FilterInitStructure.CAN_FilterIdHigh = P2_make_stdFL(filter,2);     // ID2
  CAN2_FilterInitStructure.CAN_FilterIdLow = P2_make_stdFL(filter,1);      // ID1
  CAN2_FilterInitStructure.CAN_FilterMaskIdHigh = P2_make_stdFL(filter,4); // ID4
  CAN2_FilterInitStructure.CAN_FilterMaskIdLow = P2_make_stdFL(filter,3);  // ID3
  CAN2_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
...
void UB_CAN2_std_FilterMask(CAN2_STD_FM_t filter, uint8_t nr)
{
  ...
  CAN2_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_16bit;
  CAN2_FilterInitStructure.CAN_FilterIdHigh = P2_make_stdFM(filter,3);     // ID2
  CAN2_FilterInitStructure.CAN_FilterIdLow = P2_make_stdFM(filter,1);      // ID1
  CAN2_FilterInitStructure.CAN_FilterMaskIdHigh = P2_make_stdFM(filter,4); // Maske2
  CAN2_FilterInitStructure.CAN_FilterMaskIdLow = P2_make_stdFM(filter,2);  // Maske1
  CAN2_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
  CAN2_FilterInitStructure.CAN_FilterActivation = ENABLE;
  CAN_FilterInit(&CAN2_FilterInitStructure);
}
...
void UB_CAN2_ext_FilterList(CAN2_EXT_FL_t filter, uint8_t nr)
{
  ...
  CAN2_FilterInitStructure.CAN_FilterIdHigh = P2_make_extFL(filter,2);     // ID1_Hi
  CAN2_FilterInitStructure.CAN_FilterIdLow = P2_make_extFL(filter,1);      // ID1_Lo
  CAN2_FilterInitStructure.CAN_FilterMaskIdHigh = P2_make_extFL(filter,4); // ID2_Hi
  CAN2_FilterInitStructure.CAN_FilterMaskIdLow = P2_make_extFL(filter,3);  // ID2_Lo
  CAN2_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
  CAN2_FilterInitStructure.CAN_FilterActivation = ENABLE;
  CAN_FilterInit(&CAN2_FilterInitStructure);
}
...
void UB_CAN2_ext_FilterMask(CAN2_EXT_FM_t filter, uint8_t nr)
{
  ...
  CAN2_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;
  CAN2_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;
  CAN2_FilterInitStructure.CAN_FilterIdHigh = P2_make_extFM(filter,2);     // ID1_Hi
  CAN2_FilterInitStructure.CAN_FilterIdLow = P2_make_extFM(filter,1);      // ID1_Lo
  CAN2_FilterInitStructure.CAN_FilterMaskIdHigh = P2_make_extFM(filter,4); // Mask1_Hi
  CAN2_FilterInitStructure.CAN_FilterMaskIdLow = P2_make_extFM(filter,3);  // Mask1_Lo
  CAN2_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
  CAN2_FilterInitStructure.CAN_FilterActivation = ENABLE;
  CAN_FilterInit(&CAN2_FilterInitStructure);
}
...
uint16_t P2_make_stdFL(CAN2_STD_FL_t filter, uint8_t nr)
{
  ...
  if(nr==1) { // ID1
    ...
    if((filter.id1.frame_mode==CAN2_STD_REMOTE) || (filter.id1.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x10;
    }
    if((filter.id1.frame_mode==CAN2_EXT_DATA) || (filter.id1.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x08;
    }
  }
  if(nr==2) { // ID2
    ret_wert=(filter.id2.id<<5);
    if((filter.id2.frame_mode==CAN2_STD_REMOTE) || (filter.id2.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x10;
    }
    if((filter.id2.frame_mode==CAN2_EXT_DATA) || (filter.id2.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x08;
    }
  }
  if(nr==3) { // ID3
    ret_wert=(filter.id3.id<<5);
    if((filter.id3.frame_mode==CAN2_STD_REMOTE) || (filter.id3.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x10;
    }
    if((filter.id3.frame_mode==CAN2_EXT_DATA) || (filter.id3.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x08;
    }
  }
  if(nr==4) { // ID4
    ret_wert=(filter.id4.id<<5);
    if((filter.id4.frame_mode==CAN2_STD_REMOTE) || (filter.id4.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x10;
    }
    if((filter.id4.frame_mode==CAN2_EXT_DATA) || (filter.id4.frame_mode==CAN2_EXT_REMOTE)) {
      ret_wert|=0x08;
    }
  }
...
void P2_init_CAN(void)
{
  ...
  CAN_InitStructure.CAN_Prescaler = /*CAN1_CLOCK_PRESCALER*/ 32;
...
void P2_receive(void)
{
  ...
  if(can2_buffer.status==CAN2_RX_EMPTY) {
    can2_buffer.nr_bytes=CAN2_RxMessage.DLC;
